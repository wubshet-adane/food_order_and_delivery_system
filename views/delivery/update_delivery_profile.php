<?php
        ...
        $stmt = $conn->prepare("
                        SELECT u.name as name, u.email as email, u.image as image, u.password as password, u.role as role, dp.*
                        FROM users u
                        JOIN delivery_partners dp ON dp.user_id = u.user_id
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (isset($_POST['detail_information'])) {
                ...
                //password hash
                $update_stmt = $conn->prepare("UPDATE delivery_partners SET address=?, dob=?, bank_name=?, account_name=?, account_number=? WHERE user_id=?");
    ...
    <head>
        ...
        <link rel="stylesheet" href="css/update_delivery_profile_page.css"> 
