    ...
    // Main earnings query
    $sql = "
        SELECT SUM(p.delivery_person_fee) AS total_earnings 
            FROM orders o
            JOIN payments p ON o.order_id = p.order_id
            JOIN users u ON o.delivery_person_id = u.user_id
            WHERE o.delivery_person_id = ? $dateCondition";
    ...
    foreach (['daily', 'weekly', 'monthly', 'all'] as $period) {
        ...
        $query = "
            SELECT SUM(p.delivery_person_fee) AS total 
            FROM orders o
            JOIN payments p ON o.order_id = p.order_id
            JOIN users u ON o.delivery_person_id = u.user_id
            WHERE o.delivery_person_id = ? $cond";
   ...
   // Get recent paid transactions
    function getRecentTransactions($conn, $delivery_person_id, $limit = 5) {
        $transactions = [];
        $sql = "
            SELECT 
                u.name AS customer_name, 
                r.name AS restaurant_name, 
                p.delivery_person_fee AS total_amount, 
                o.order_date 
            FROM orders o
            JOIN payments p ON o.order_id = p.order_id
        ...
        ";
        if ($stmt = $conn->prepare($sql)) {
            $stmt->bind_param("ii", $delivery_person_id, $limit);
            $stmt->execute();
            $res = $stmt->get_result();
    ...
    $transactions = getRecentTransactions($conn, $delivery_person_id);
    ?>
    <!--js chart-->
