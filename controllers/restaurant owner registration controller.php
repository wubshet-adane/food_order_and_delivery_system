<?php
require_once __DIR__ . '/../models/restaurant_owner_registration_Model.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $national_id_front = handleFileUpload('id_proof_front', $upload_dir);
    $national_id_back = handleFileUpload('id_proof_back', $upload_dir);

    $result = MerchantRegister::restaurantRegistrationfunction([
        'fullname' => $fullname,
        'role' => $role,
        'password' => $password,
        'dob' => $dob,
    ...
    if ($result) {
        ...
        header("Location: ../views/auth/restaurant_owner_registration.php?error=Email already registered.");
        exit;
    }else {
        header("Location: ../views/auth/restaurant_owner_registration.php?error=Registration failed. Please try again.");
        exit;
    }
}
